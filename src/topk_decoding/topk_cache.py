...
class TopkCache(Cache):
    ...
    def create_key_database(key_states, flat=True, BH=None, D=None):
        ...
        for i in range(BH):
            if flat:
                ...
                search_index = faiss.IndexIVFFlat(quantizer, D, nlists, faiss.METRIC_INNER_PRODUCT)
                if N < 100000:
                    search_index.train(key_states[i, :, :].contiguous().to(torch.float32).cpu())
                else:
                    search_index.train(key_states[i, :100000, :].contiguous().to(torch.float32).cpu())
                search_index.add(key_states[i, :, :].contiguous().to(torch.float32).cpu())
    ...
    @staticmethod
    def load_cache_directory(directory: str = None, flat: bool = True) -> 'DynamicFaissCache':
        cache = DynamicFaissCache()
        cache.flat = flat
        files = os.listdir(directory)

        key_files = sorted([f for f in files if f.startswith("key_") and f.endswith(".pt")], 
                           key=lambda x: int(x.split("_")[1].split(".")[0]))
        value_files = sorted([f for f in files if f.startswith("value_") and f.endswith(".pt")], 
                             key=lambda x: int(x.split("_")[1].split(".")[0]))
        ...
        for key_file, value_file in tqdm(zip(key_files, value_files)):
            ...
            key_db = DynamicFaissCache.create_key_database(flat=flat, key_states=key_tensor)
    ...
    @staticmethod
    def load_cache_tensor(filepath: str = None, tensor: torch.Tensor = None, flat: bool = True, load_as_directory=False) -> 'DynamicFaissCache':
